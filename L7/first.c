...
{ /* Funcționalitate: realizarea secvenței de operații de actualizare a fișierului de date. */
	...
	for(i = 1; i < n; i++) // Iterăm prin secvența de numere reale specificată în linia de comandă.
	{
        ...
        if(valoare < 0 && (stoc + valoare) < 0)
        {
            printf("Doriti sa alimentati cu %f lirti de bezina? y/n\n", stoc);
            char optiune = getchar();
            if(optiune == 'y')
            {
                stoc = 0;
            }
            ...
            {
                if(optiune == 'n')
                    printf("Nu a fost alimentat autoturismul.\n");
            }
        }
        else
        {
        ...
        if(-1 == fcntl(fd,F_SETLK,&myLock) ) { perror("Eroare la deblocaj"); exit(9); }
        myLock.l_type = F_WRLCK;
